...
Pois é, adeus Tuber e bem-vindo ao time de engenharia do Google! Agora você terá que criar uma função no arquivo pratica-16.js que receba dois parâmetros: • a lista de fotos • o termo de busca

Com os dados de entrada fornecidos, retorne os itens que correspondem a busca.

Dados de entrada da função: Lista de fotos:
[{
...
let fotos = [{
  ...
  const procuraFotos = (lfotos, termo) => {
    let lfotosfiltrada = []
   lfotosfiltrada = lfotos.filter(function(item){
    return item.tags.includes(termo);
   });

   return lfotosfiltrada

  }

  console.log(procuraFotos(fotos, "cachorro"));
